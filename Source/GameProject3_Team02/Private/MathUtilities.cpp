 ...
 */
FRotator FMathUtilities::CombineRotators(FRotator RotatorA, FRotator RotatorB)
{
...
FVector FMathUtilities::MoveTowards(FVector Current, FVector Target, float MaxDeltaDistance)
{
...
FQuat FMathUtilities::LookRotation(FVector Forward, FVector Upward)
{
...
FQuat FMathUtilities::ShortestRotation(FQuat A, FQuat B)
{
	if (Dot(A, B) < 0.f)
	{
		return A * (-B).Inverse();
...
float FMathUtilities::Dot(FQuat A, FQuat B)
{
